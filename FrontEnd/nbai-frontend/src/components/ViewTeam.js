...
class ViewTeam extends Component {
  ...
  componentDidMount() {
    fetch(`${Config.API}/Allocations`)
      .then(res => res.json())
      .then(data => {
        this.setState({ allocations: data });
      });

    fetch(`${Config.API}/Players`)
      .then(res => res.json())
      .then(data => {
  ...
  render() {
    ...
    let teamAllocations = this.state.allocations.filter(
      alloc => alloc.team === teamName
    );
    let teamPlayerNames = teamAllocations.map(alloc => alloc.player);
    let teamPlayers = this.state.players.filter(player =>
      teamPlayerNames.includes(player.player1)
    );
    ...
    return (
      <div>
        ...
        <table className="ViewTable">
          <thead>
            <tr>
              {Object.keys(teamPlayers[0] || {}).map(col => (
                <th>{KEY_HUMAN_MAP[col] ? KEY_HUMAN_MAP[col] : col}</th>
              ))}
            </tr>
          </thead>
          <tbody>
        ...
        <Link to={`/EditTeam/${teamName}`}>
          <button className="btnChangePage">Edit Team</button>
        </Link>
      </div>
    );
  }
