          ...
          <tbody style={{ overflow: "auto", maxHeight: "100vh" }}>
            {playersPaginated.map(player => (
              <tr key={player.player1}>
                <button
                  type="button"
                  onclick={e => this.AddPlayer(player.playerId, e.target.value)}
                >
                  +
                </button>
                ...
                {/* <Checkbox
                  onChange={e =>
                    this.handleCheckbox(player.playerId, e.target.value)
                  }
                  checked={this.state.allocations.includes(player.playerId)}
                /> */}
